...
ht <- hux(`#`=seq(1:12),
    Field=c("maker", "ordinal","timestamp", "mks", "state", "price", "volume", "change", "trade","taker","heard", "deleted" ),
    Type=c("UUID", "long", "string", "int", "bool", "double", "double", "double", "long long", "UUID", "int", "bool" ),
    Description=c("UUID of the order open on the order book",
                  "A sequential number of the event for this order",
                  "Time the event occurred on the exchange in ISO 8601 format, up to a second. Empty field means that the value is the same as in the previous event (previous row)",
                  "Microseconds part of the time the event occurred on the exchange",
                  ...
                  "If the event is originated by a trade it is an exchange-generated unique trade identification number; otherwise emtpy. Thus $m_k^i \\equiv$ trade isn't empty",
                  "If the event is originated by a trade, it is an UUID of the matched taker order; otherwise empty",
                  "Microseconds passed since the event occurred on the exchange till receving by application",
  ...
  set_bold(2:13, 2) %>%
  set_caption("Era file fields and their corresponding elements of order event with order id tuple as defined by \\@ref(def:order-set-table). If the value of a field can not be determined, it is reported as NA.") %>%
  set_caption_pos("topleft")
print_html(ht)
```
