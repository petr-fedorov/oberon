...
Reconstruction algorithm comprises extract and cleanse parts. Figure \@ref(fig:reconstruction-activity) shows UML2 Activity Diagram [@cook_omg_2017] of a standalone application where interaction of the algorithm' parts can be seen.  The application consists of two classes: 

* `Application`, that provides data for the algorithm, and,
* `Reconstructor` shown in light green, that implements the algorithm


```{r reconstruction-activity, echo=FALSE, fig.cap="UML2 Activity Diagram  of a standalone application using Reconstruction algorithm", fig.align="center"}
knitr::include_graphics("reconstruction.png", dpi=80)
```
`Application` class starts from `S.1 Initialize` activity.  The activity connects to a websocket of the exchange specified by `exchange_name` and subscribes for the channels sending messages about the pair specified by `pair_name`. Then `S.1 Initialize` creates a store (i.e. file) where reconstructed events is to be persisted, creates an instance of `Reconstructor` and passes the store to the instance for future use. 

Aftewards `Application` launches `S.2 Recive messages` and `S.1 Extract`. These activities run concurrently. `S.2 Recive messages` constinously receives messages from the exchange (note `{stream}` on the figure \@ref(fig:reconstruction-activity)) and sends them to `S.1 Extract` for a decoding (see figure \@ref(fig:reconstruction-use-case) for the list of decoders) and an accumulation to be processed later by `S.1 Cleanse`.

The activity `S.1 Cleanse`, which is called periodically, performes so called *data cleansing*. According to [@rahm_data_2000]:
...
UML2 UseCase Diagram [@cook_omg_2017] shown on figure \@ref(fig:reconstruction-use-case) lists the kinds of cleansing performed by `S.1 Cleanse`. It should be clear from this list why `S.1 Cleanse` needs to be called periodically rather than processing each message as soon as the message has arrived.
...
The output of `S.1 Cleanse` is a set of order events as defined by \@ref(def:order-event) which together compose Limit Order Book as defined by \@ref(def:lob)
