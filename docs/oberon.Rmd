...
First we need to define what we reconstruct. Surprisingly, the concept of order and operations of a continous double auction are rather complicated things if one attempts to define them rigorously! Consider, for example,  a definition given in the book [@bouchaud_trades_2018]:
...
It might not be obvious, but above definition and description are incomplete:
...
Note that $\text{supp}(f_{v})$ is a set of càdlàg discontinuity points of $v(t)$ while $\text{supp}(a_{v})$ is a set of càglàd discontinuity points of $v(t)$.
...
as_hux(sot[order(t),
  ...
  add_colnames() %>%
  set_align(everywhere, c(1:4), 'center') %>%
  set_align(1, c(5:8), 'right') %>%
  set_right_border(everywhere,1, 1) %>%
  set_caption('(#tab:example-order-table) The sample order table contains information about six limit orders, three market orders and six trades.') %>%
...
An *order table* is a set of tuples $$\{(i, n, t^i_n,e^i_n, p^i_n, v^i_n,f^i_n)\}_{n=1}^{N^i}$$ called *order events* where

a. $i \in \mathbb{N}$ is an *unique order identification*,
a. $n \in \{1, \ldots, N^i\}$ is an *order event number*, 
b. $t^i_n \in \mathbb{R}_{>0}$ is an *event timestamp*, $\forall n: t^i_n < t^i_{n+1}$, 
b. $e^i_n \in \{0, 1\}$ is a *event type*; $e^i_n = 1$ is an *activation* event, $e^i_n = 0$ is an *deactivation* event,
b. $e^i_1 = 1$ and $\forall n \in \{2, \ldots, N^i\} : e^i_n = 0 \implies e^i_{n-1} = 1$,
b. $p_n^i \in \mathbb{R}_{>0}$ is an *order price*,
b. $v_n^i \in \mathbb{R}$ is an *order volume*,
b. $\forall n_1, n_2: \text{sgn}(v_{n_1}^i) = \text{sgn}(v_{n_2}^i)$,
b. $f^i_n \in \{0, 1\}$ is a *fill indicator*; $f^i_n = 1$ indicates that the volume is changed due to fill.
```

We say that an order $i$ event $n$ with $e^i_n = 1$ is an *activation* event in a sense that after the event $n$  order $i$ is *active*, i.e. represents a commitment to buy or sell given volume $v_n^i$ of an asset at no worse than  a price $p^i_n$. If $b^i_n = 1$ then the commitment is valid at $t_n^i$ and afterwards. If $b^i_n = 0$ then the commitment is valid after $t_n^i$. 

Similarly, we say that an event $m$ with $e^i_m = 0$  is a *deactivation* event in a sense that it indicates the last moment when the commitment to buy or sell $v^i_m$ at no worse than $p^i_m$ is still valid. If $b^i_m = 1$ then the commitment is valid at $t_m^i$ and not valid afterwards. If $b^i_m = 0$ then the commitment is valid before $t_m^i$ and not valid at $t_m^i$ and afterwards. 
...
A set of orders with common resolution parameters $v_0$ and $\vartheta$ $\mathcal{O} = \{o^i(t)\}_i$ where $o^i(t)$ is defined by \@ref(def:order) is called an *order book*. A (possibly empty) subset $\mathcal{B} =\{o^b(t)\}_{v_k^b > 0} \subset \mathcal{O}$ of buy orders is called *bids* and  a (possibly empty) subset $\mathcal{A} =\{o^a(t)\}_{v_k^a < 0} \subset \mathcal{O}$ of sell orders is called *asks*.
```
...
Suppose a Limit Order Book $\mathcal{L}$ is given and $\mathcal{A} =\{o^a(t)\}_{v_k^a < 0} \subset \mathcal{L}$ as defined by \@ref(def:order) and \@ref(def:lob). Suppose that at time $t_j$ $$\sum_{o^a(t) \in \mathcal{A} } f^a(t_j) < 0 $$ Then it is said that a *market buy order* is placed and executed at time $t_j$ for $\sum_{o^a(t) \in \mathcal{A} } |f^a(t_j)p^a(t_j)|$ units of currency.  

```


```{definition Market Sell Order, name="Market Sell Order", label=market-sell-order}
Suppose a Limit Order Book $\mathcal{L}$ is given and $\mathcal{B} =\{o^b(t)\}_{v_k^b > 0} \subset \mathcal{L}$ as defined by \@ref(def:order) and \@ref(def:lob). Suppose that at time $t_j$ $$\sum_{o^b(t) \in \mathcal{B} } f^b(t_j) > 0 $$ Then it is said that a *market sell order* is placed and executed at time $t_j$ for $\sum_{o^b(t) \in \mathcal{B} } f^a(t_j)$ units of asset.  
...
To make the above definitions intuitively clear, let's consider an example. Table \@ref(tab:example-order-table) contains a consolidated order table for six limit orders and figure \@ref(fig:order-life-cycle) shows all possible ways of processing an order by an exchange (though not every exchange supports all operations shown).

```{r order-life-cycle, echo=FALSE, fig.cap="All possible ways of processing an order by an exchange.", fig.align="center"}
knitr::include_graphics("order-life-cycle.png")
```
...
as_hux(sot[order(t),
  ...
  set_width(0.9) %>%
  set_align(everywhere, c(1:4), 'center') %>%
  set_align(1, c(5:9), 'right') %>%
  set_right_border(everywhere,1, 1) %>%
  set_left_border(everywhere,9, 1) %>%
  set_caption('(#tab:example-order-table) The sample order table contains information about six limit orders, three market orders and five trades.') %>%
  set_caption_pos("topleft") %>%
  set_position("left")
...
v <- ggplot(mapping=aes(x=t, y=v, colour=as.character(i),  group=as.character(i))) + 
  ...
  geom_point(data=rbind(sot.intervals[e==1 & v != p.v & f == 1,  .(t = t_1, v, e, i)],
...
```{sql sql - coinbase.quotes, eval=FALSE, echo=FALSE, connection=con, output.var=coinbase.quotes}
select * 
from coinbase.full
where _type != 'match'
...
```{sql sql - coinbase.trades, eval=FALSE, echo=FALSE, connection=con, output.var=coinbase.trades}
select * 
from coinbase.full
where _type = 'match'
order by _time, _type
