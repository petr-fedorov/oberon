...
Below we propose stricter definitions that address these and the other similar issues while staying as close as possible to [@bouchaud_trades_2018]. Afterwards we describe the reconstruction process.
...
Suppose an order function $o(p,t)$ as defined by \@ref(def:order) is given. A set $$\{(t_k',s_k',p_k',v_k', m_k')\}_k$$ of $K$ or $K+1$ order events 
...
```{proof}
Suppose the order table $\{(t_k',s_k',p_k',v_k', m_k')\}_{k=1}^{R}$ is given. To restore its order function as defined by \@ref(def:order) we need to restore $\mathcal{T}$, $\mathcal{P}$ and $\mathcal{V}$ sets.

$\mathcal{T}$ is defined as follows:

 a. For all $1 \leq k < R$ the interval $T_k$ has left endpoint $t_k'$ and right endpoint $t_{k+1}'$ and is:
    * a left-open if $m_k'=0$ and left-closed if $m_k'=1$
    * a right-open if $m_{k+1}'=1$ and right-closed if $m_{k+1}'=0$
    
 a. If $s_R' = 1$ then the interval $T_R$ is added to $\mathcal{T}$ which is either $(t_R', +\infty)$ if $m_R'=0$ or $[t_{R}', +\infty)$ if $m_R'=1$.

Then $\mathcal{P} = \{ \{p_k'\}\}_{k=1}^{|\mathcal{T}|}$ and $\mathcal{V} = \{ v_k'\}_{k=1}^{|\mathcal{T}|}$
...
Data cleansing is performed by a sequence of `TokenHandler`s as shown on figure \@ref(fig:sd-tokenhandler-handle). Below lists of `TokenHandler`s used for different exchange event streams are shown.
...
The corresponding filled volume function is identically zero: $$f_v(t) \equiv 0$$

The corresponding amended volume function is non-zero at just one point: $$f_a(\text{`r events$t[1]`}) = 1.0$$
...
print_html(format_table(events, "The order was matched after it was open on the order book ($m_2 = 1$) and afterwards its price and volume were simultaneously amended ($m_3 = 0$, $s_3=1$)."))
```
