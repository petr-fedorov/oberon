...
```
The application starts from `S.7 Initialize` activity.  The activity connects to a websocket feed provided by the exchange specified by `exchange_name` parameter and subscribes for channels sending messages about the pair specified by `pair_name` parameter. Then `S.7 Initialize` creates a store (for example, a file) where the reconstructed events is to be persisted, creates an instance of `Reconstructor` object and passes the store to the instance for the future use. 

Afterwards the application launches `S.5 Receive messages` and `R.1 Reconstruct Order Book`. These activities run concurrently. `S.5 Receive messages` continuously receives messages from the exchange (note `{stream}` on the figure \@ref(fig:reconstruction-activity)) and stores them into `buffer`.  `R.1 Reconstruct Order Book` takes messages from the `buffer` in FIFO order and processes them as shown on figure \@ref(fig:reconstruction-activity)
...
as_hux(bitfinex.extracted[,
  ...
  set_right_border(everywhere,1, 1) %>%
  set_na_string(everywhere, everywhere, "NA") %>%
  set_caption('(#tab:stamp-quote-frag) A fragment of ETHUSD stream extracted from data generated by Bitfinex during 1 second. Similar to Bitstamp, Bitfinex sends duplicate events and the correct class of an event can not always be determined. Furthermore, many attributes are missing and need to be deduced by a cleansing process.') %>%
  set_caption_pos("topleft")
...
bitstamp.cleansed <- reconstruct("Bitstamp", bitstamp.quotes[microtimestamp <= '2020-03-25 12:17:24+03',], bitstamp.trades[trade_timestamp <= '2020-03-25 12:17:24+03',], extract.only = FALSE)[lubridate::ceiling_date(timestamp) == '2020-03-25 12:17:24+03',]
bitstamp.cleansed <- bitstamp.cleansed[order(local_timestamp)]
setDT(bitfinex.quotes)
setDT(bitfinex.trades)
