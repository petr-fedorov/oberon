...
as_hux(coinbase.extracted[order(local_timestamp), 
                    .(`#`=.I,
  ...
  set_caption('(#tab:stamp-quote-frag) A fragment of ETHUSD stream extracted from data generated by Coinbase during 1 second. Event numbers \\(n\\) and volume \\(v^i_n\\) need to be deduced.') %>%
  set_caption_pos("topleft")
...
as_hux(bitstamp.extracted[order(local_timestamp), 
                    .(`#`=.I,
                    `\\(i\\)(maker)`=substr(maker_order_id,25, 36),
                    `\\(n\\)`=event_no,
                    `\\(t^i_n\\)`=format(timestamp,format="%H:%M:%OS3"),
  ...
  set_right_border(everywhere,1, 1) %>%
  set_na_string(everywhere, everywhere, "NA") %>%
  set_caption('(#tab:stamp-quote-frag) A fragment of ETHUSD stream extracted from data generated by Bitstamp during 1 second.') %>%
  set_caption_pos("topleft")
...
as_hux(bitfinex.extracted[order(local_timestamp), 
                    .(`#`=.I,
                    `\\(i\\)(maker)`=substr(maker_order_id,25, 36),
                    `\\(n\\)`=event_no,
                    `\\(t^i_n\\)`=format(timestamp,format="%H:%M:%OS3"),
...
coinbase.extracted <- reconstruct("Coinbase", coinbase.full[lubridate::ceiling_date(`_time`) == '2020-04-01 16:51:57',], extract.only=TRUE)
setDT(bitstamp.quotes)
setDT(bitstamp.trades)
bitstamp.extracted <- reconstruct("Bitstamp", bitstamp.quotes[lubridate::ceiling_date(microtimestamp) == '2020-03-25 12:17:24+03',], bitstamp.trades[lubridate::ceiling_date(trade_timestamp) == '2020-03-25 12:17:24+03',], extract.only = TRUE) 

setDT(bitfinex.quotes)
setDT(bitfinex.trades)
bitfinex.extracted <- reconstruct("Bitfinex", bitfinex.quotes[lubridate::ceiling_date(exchange_timestamp) == '2020-03-26 07:01:34+03',],
                                  bitfinex.trades[lubridate::ceiling_date(exchange_timestamp) == '2020-03-26 07:01:34+03'], extract.only = TRUE) 
save( coinbase.extracted, bitstamp.extracted, bitfinex.extracted, file="oberon.Rdata", compress=TRUE)
