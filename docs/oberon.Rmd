...
Figure \@ref(fig:reconstruction-flowchart) shows UML2 Activity Diagram of the reconstruction algorithm (see [@cook_omg_2017] for the meaning of the shapes):

```{r reconstruction-flowchart, echo=FALSE, fig.cap="An overview of the reconstruction algorithm", fig.align="center"}
knitr::include_graphics("reconstruction.png", dpi=80)
```
The content of *Records Received* depends on an exchange and type of a record, it may describe an order-related event or trade.  From these records we *Extract* information in the standard format for an order event as defined by \@ref(def:order-event) and a trade as defined by \@ref(def:trade). Then we  *Integrate* these objects and in particular attempts to find for each trade a corresponding order event. The output event is Order Event as defiind by \@ref(def:order-event) and collectively at this stage the events satisfy constraints of Order Table as defined by \@ref(def:order-table). Order Table may not satisfy constraints of Limit Order Book as defined by \@ref(def:lob) yet, so it is *Cleansed* [@rahm_data_2000] and, in particular, we attempt to identify market orders as defined by \@ref(def:market-buy-order) and \@ref(def:market-sell-order). See [oberon Documentation](design/index.html) for additional info.
