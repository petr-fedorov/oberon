...
```
The content of Order and Trade Records depends on an exchange.  From these records we *extract* information in the standard format for an order event as defined by \@ref(def:order-event) and a trade as defined by \@ref(def:trade). These objects are then *integrated* (i.e. for each trade a corresponding order event is found) to produce Order Event[Integrated]. Collectively Order Event[Integrated] form Order Table as defined by \@ref(def:order-table). Order Table may not satisfy constraints of Limit Order Book as defined by \@ref(def:lob) yet, so it is *cleansed* [@rahm_data_2000]. Collectively Order event[Cleansed] constitute Limit Order Book.
