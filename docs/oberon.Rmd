...
```
The activity node `Process message` on figure \@ref(fig:reconstruct-order-book) is CallAction. It calls method `process()` on `target` instance of class `Reconstructor` as shown on figure \@ref(fig:reconstructor-hierarchy-process).  
...
```{r sd-tokenhandler-handle, echo=FALSE, fig.cap="UML2 Sequence diagram showing a \"happy path\" trace of TokenHandler::handle() method. The loop implements \"iterative\" ExpansionRegion from figure \\@ref(fig:ad-reconstructor-process)", fig.align="left"}
