...
Figure \@ref(fig:reconstruction-flowchart) shows a flowchart of the reconstruction algorithm (see [@iso_5807_information_1985] for the meaning of the shapes):

```{r reconstruction-flowchart, echo=FALSE, fig.cap="An overview of the reconstruction algorithm", fig.align="center"}
knitr::include_graphics("reconstruction.png")
```
The content of Order and Trade Streams depends on an exchange.  From these streams we *extract* information in the standard format for an order event as defined by \@ref(def:order-event) and a trade as defined by \@ref(def:trade). This information is *integrated* (i.e. trades and their corresponding order events must be coupled) to produce Order Table as defined by \@ref(def:order-table). The order table may not satisfy constraints of Limit Order Book as defined by \@ref(def:lob), so it needs to be *cleansed* [@rahm_data_2000]. Limit Order Book Stream consists of order events which collectively satisfy constraints of Order Table and Limit Order Book.

Let's start the detailed description of the algoritm from the definition of what we reconstruct. Surprisingly, an order,  a limit order book,  the operations of a continous double auction are rather complicated things if one attempts to define them rigorously. Consider, for example,  a definition given in the recent book [@bouchaud_trades_2018]:
...
As it has already been mentioned above an order is a commitment at a given time, to buy or sell a given volume of an asset at no worse than a given price. Figure \@ref(fig:order-life-cycle) shows [the state diagram](https://en.wikipedia.org/wiki/State_diagram) of an order while the order is being processed by an exchange.  The possible states an order can be in are:

 * *Received* - a valid order has been received by an exchange and is now active, i.e. it may be filled. This state is fleeting.
 * *Open* - the order is now open on the order book. Orders which are not fully filled or cancelled immediately enters this state. This state is lasting.
 * *Done* - the order is no longer on the order book. This state can result from an order being canceled or filled. This state is fleeting.
...
If an order has been *Partially Filled* before entering *Open* state or it has been *Filled* or *Partially Filled* after *Price Amendment*, then the order is sometimes called a market limit order.  We always model market limit orders as a comibnation of an appropriate market order and a limit order that placed simultaneously. 
