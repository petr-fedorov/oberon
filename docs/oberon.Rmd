...
A function $o : \mathbb{R}^2_{\geq 0} \mapsto \mathbb{R}$ is called a limit *order function* if it can be written as 
$$ 
  o(p,t) = \sum_{k=1}^K o_k \mathbf{1}_{P_k \times T_k}(p,t)
$$
where  

   a. $\mathcal{T} = \{T_k\}_{k=1}^{K}$ is a set of proper intervals of $\mathbb{R}_{\geq 0}$ with endpoints $t_{k,1} \in \mathbb{R}_{\geq 0}$, $t_{k,2} \in \mathbb{R}_{\geq 0} \cup \{+\infty\}$, such that:
      i. $t_{k,1} < t_{k,2}$
      i. $t_{k,1},t_{k,2} \in \{n \tau : n \in \mathbb{N}\}$ where $\tau \in \mathbb{R}_{>0}$ is a constant called *sample interval* 
      i. $\forall k_1 \neq k_2 : T_{k_1} \cap T_{k_2} = \emptyset$, 
 
a. $\mathcal{P} = \{P_k\}_{k=1}^{K}$ is a set of degenerate intervals $\{p_k\}$ with endpoints $p_k \in \mathbb{R}_{\geq 0}$, such that $\forall k: p_k \in \{ n \pi : n \in \mathbb{N} \}$ where $\pi \in \mathbb{R}_{>0}$ is a constant called *quote increment* or *tick size*,
 
 a. $\mathbf{1}_{P_k \times T_k}(\cdot,\cdot)$ is an indicator function of the set ${P_k \times T_k}$: 
$$
\mathbf{1}_{P_k \times T_k}(p,t) = \begin{cases} 1, & (p,t) \in P_k \times T_k \\ 0, & (p,t) \not \in P_k \times T_k  \end{cases}
$$
 a. $o_k \in \mathbb{R}$ are constants called *order volume* such that either $o_k \in \mathbb{R}_{\geq 0}$ (buy order) or $o_k \in \mathbb{R}_{\leq 0}$ (sell order) for all $k$ and $|o_k| \in \{ n \phi : n \in \mathbb{N}\}$ where $\phi \in \mathbb{R}_{>0}$ is a constant called *base increment* or *lot size*, 
 a. $\forall p$ the function of time $o_p(t) \equiv o(p,t)$ is either càdlàg or càglàd at each point of discontinuity.
...
Note that we follow [@bouchaud_trades_2018] while modelling order amendments and, in particular, order placement, as càglàd (continue à gauche, limite à droite) process:
...
Trades are modeled as *càdlàg (continue à droite, limite à gauche)* occurrences so they are visible immediately upon arrival. If an order is fully executed upon arrival, it will never be shown in order book.

The consequence of the above is that $v(t)$ can be interpreted as a amount available in order book for trade at time $t$ as it should.
...
A set of orders with common resolution parameters $\tau$, $\pi$ and $\phi$ $$\mathcal{O} = \{o^i(\cdot)\}_i$$ where $o^i(\cdot)$ is defined by \@ref(def:order) is called an *order book*. 
...
Figure \@ref(fig:oberon-overview) shows UML2 Activity Diagram [@cook_omg_2017] of `Standalone application - capture` activity that represents an implementation of `oberon capture` command. The activity node `Reconstruct Order Book` performs a reconstruction of order book events from exchange-specific messages that are passed by node `Receive messages`.


```{r oberon-overview, echo=FALSE, fig.cap="`Receive messages` activity node collects avaialbe information in an exchange-specific format and pass the information in the form of [Boost.Property Tree](https://www.boost.org/doc/libs/1_73_0/doc/html/property_tree.html) to `Reconstruct Order Book` activity node that, as its name implies, reconstructs order book events in an *universal* format described herein. Note that both activity nodes run concurrently.", fig.align="left"}
knitr::include_graphics("Standalone application - capture.png", dpi=80)
```

Figure \@ref(fig:reconstruct-order-book) shows UML2 Activity Diagram of `Reconstruct Order Book` activity which is called by the activity node with the same name on figure \@ref(fig:oberon-overview).
...
```{r message-hierarchy-process, echo=FALSE, fig.cap="\"Message\" hierarchy of classes used in implementation of \"Process message\" activity.", fig.align="center"}
...
Handling of meessage by `TokenHandler` is implemented using [Visitor](https://en.wikipedia.org/wiki/Visitor_pattern) pattern as shown by UML2 Sequence diagram on figure \@ref(fig:sd-message-accept).
...
### Coinbase data cleansing

  * Coinbase::Syncrhonizer
  
  * SizeDeducer
  * EventCounter
  * File

### Bitstamp data cleansing

  * Bitstamp::Synchronizer
  * Bitstamp::TimeOrderEnforcer
  * Bitstamp::Deduplicator
  
  * SizeDeducer
  * EventCounter
  * File

### Bitfinex data cleansing
