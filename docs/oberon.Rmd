...
The activity `R.2 Extract` takes an exchange-specific data in [Boost.PropertyTree](https://www.boost.org/doc/libs/1_73_0/doc/html/property_tree.html) container and  produces one or more internal exchange-independent messages shown on figure \@ref(fig:message-hierarchy). 
...
If an order has been *matched* before entering *Maker* state or it has been *matched* after *price amendment*, then the order is sometimes called a market limit order.  We always model market limit orders as a comibnation of an appropriate market order and a limit order that placed simultaneously. 
...
Coinbase sends correct data so cleansing is not needed.
...
as_hux_event_table(bitstamp.extracted,'(#tab:stamp-quote-frag) A fragment of ETHUSD stream extracted from data generated by Bitstamp during 1 second.')

```

Bitstamp fragment has the following inconsistencies which need to be clenased:

  * Duplicated events
  * Takers enters order book as well as Makers

```{r Bitstamp Duplicated Events Example, eval=TRUE, echo=FALSE}
as_hux_event_table(bitstamp.extracted,'(#tab:stamp-duplicated) An example of duplicated events in ETHUSD stream extracted from data generated by Bitstamp.', 15:16)

```

```{r Bitstamp Takers in Order Book Example, eval=TRUE, echo=FALSE}
as_hux_event_table(bitstamp.extracted,'(#tab:stamp-duplicated) An example of takers entering order book in ETHUSD stream extracted from data generated by Bitstamp.', 17:18)
...
as_hux(bitfinex.extracted[,
  ...
  set_right_border(everywhere,1, 1) %>%
  set_na_string(everywhere, everywhere, "NA") %>%
  set_caption('(#tab:stamp-quote-frag) A fragment of ETHUSD stream extracted from data generated by Bitfinex during 1 second.') %>%
  set_caption_pos("topleft")
...
as_hux_event_table(bitstamp.cleansed,'(#tab:stamp-cleansed-quote-frag) A fragment of ETHUSD stream extracted from data generated by Bitstamp during 1 second and cleansed.')
```
...
setDT(bitstamp.quotes)
setDT(bitstamp.trades)
bitstamp.extracted <- reconstruct("Bitstamp",
                                  bitstamp.quotes[microtimestamp <= '2020-03-25 12:17:24+03',],
                                  bitstamp.trades[trade_timestamp <= '2020-03-25 12:17:24+03',],
                                  extract.only = TRUE)[lubridate::ceiling_date(timestamp) == '2020-03-25 12:17:24+03',]

bitstamp.cleansed <- reconstruct("Bitstamp",
                                  bitstamp.quotes[microtimestamp <= '2020-03-25 12:17:24+03',],
                                  bitstamp.trades[trade_timestamp <= '2020-03-25 12:17:24+03',],
                                  extract.only = FALSE)[lubridate::ceiling_date(timestamp) == '2020-03-25 12:17:24+03',]
setDT(bitfinex.quotes)
setDT(bitfinex.trades)
...
save( coinbase.extracted, bitstamp.extracted, bitstamp.cleansed, bitfinex.extracted, file="oberon.Rdata", compress=TRUE)
