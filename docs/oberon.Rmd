...
library(gridExtra)
library(latex2exp)
options(scipen = 999, digits = 6, digits.secs=3)
...
```{proof Order Table Equivalence proof}
...
A finite set of orders with common resolution parameters $\tau$, $\pi$ and $\phi$ $$\mathcal{O} = \{o^i(\cdot, \cdot)\}_i$$ where $o^i(\cdot, \cdot)$ is defined by \@ref(def:order) is called an *order set*.
```
...
```{definition LOB function, name="LOB function", label="lob-function"}
Suppose Limit Order Book $\mathcal{L}$ as defined by \@ref(def:lob) is given, $\mathcal{B}$ is an associated *bids* set as defined by \@ref(def:bids) and $\mathcal{A}$ is an associated *asks* set as defined by \@ref(def:asks). The functions
$$
  o^{\mathcal{B}}(p,t) = \sum_{o^i(p,t) \in \mathcal{B}} o^i(p,t) \\
  o^{\mathcal{A}}(p,t) = \sum_{o^i(p,t) \in \mathcal{A}} o^i(p,t) 
$$
are called *LOB bid function* and *LOB ask function* respectively. Collectively they are called *LOB functions*. 
```

Note that $$\text{supp}(o^{\mathcal{A}}(p,t)) \cap \text{supp}(o^{\mathcal{B}}(p,t)) = \emptyset$$ but $\overline{\text{supp}(o^{\mathcal{A}}(p,t))} \cap \overline{\text{supp}(o^{\mathcal{B}}(p,t))}$ may be non-empty and in this case it will be a finite set of points.
...
### Level2 order function


The Order Set Table is a precise representation of Order Functions but it is not a good tool for analysis. Consider for example the really short table \@ref(tab:transmute-example-1). Even in this simple case, it is rather hard to see how LOB was evolving. The visual representation of the same Order Set Tableshown on figure \@ref(fig:transmute-example-1-picture) is much more expressive.
...
as_hux(dt[ , .(`\\(i\\)`=maker, `\\(k\\)`=ordinal, `\\(t^i_k\\)`=timestamp+mks/1000000.0+0.0000005,
               `\\(s^i_k\\)`=state, `\\(p^i_k\\)`=price, `\\(v^i_k\\)`=volume, `\\(m^i_k\\)`=as.integer(trade != 0) )]) %>%  set_caption("Despite being short, the Order Set Table is not comprehensible; it is difficult to figure out from the table how the order book was evolving.")
```
...
gnuplot -p <<-EOF
  ...
  plot \
    "transmute_example_1.csv" using \
      ...
      (0):\
      (val("volume") - val("volume_change")) with vectors lc palette z lw 4 nohead notitle,\
    '' using\
      ((t=tval(t),t+val("mks")/1000000.0)):\
...
```{r transmute-example-1-picture, echo=FALSE, fig.cap="It is easier to grasp the evolution of an order book from the visual representation of Order Set Table \\@ref(tab:transmute-example-1) shown above", fig.align="center"}
knitr::include_graphics("transmute_example_1.png", dpi=80)
```

Unfortunately, not every Order Set Table can be visualised 'as-is'. Suppose that one more order function is added to LOB as shown in table \@ref(tab:transmute-example-2). Its visualisation shown on figure \@ref(fig:transmute-example-2-picture) is distorted since the lines corresponding to orders 3 and 4 overlaps. These orders have the same price and their time intervals intersects.
...
as_hux(dt[ , .(`\\(i\\)`=maker, `\\(k\\)`=ordinal, `\\(t^i_k\\)`=timestamp+mks/1000000.0+0.0000005,
  ...
  set_caption("Order 4 with two events and the same price as order 3 (540) has been added to Order Set Table.")
...
```{r transmute-example-2-picture, echo=FALSE, fig.cap="A visual representation of Order Book Set \\@ref(tab:transmute-example-2) is distorted since the lines representing orders 3 and 4 overlap", fig.align="center"}
knitr::include_graphics("transmute_example_2.png", dpi=80)
```
This example is a motivation for theorem \@ref(thm:l2of). The theorem guarantees that we may transmute Order Set Table into another one, which at every moment and for every price contains only one non-zero order function.

```{theorem Level2 order, name="Level2 Order Function", label=l2of}
    Suppose Limit Order Book $\mathcal{L}$ as defined by \@ref(def:lob) is given,  $o^{\mathcal{L}}(p,t)$ is LOB function as defined by \@ref(def:lob-function) (either LOB bid function or LOB ask function) and for some constant $p_0 \in \mathbb{R}_{>0}$:  $$\emptyset \neq \text{supp}(o^{\mathcal{L}}(p_0,t)) = \bigcup_{m=1}^M I_m$$ where all $I_m$ are intervals such that $\forall i,j: \overline{I_i} \cap \overline{I_j} = \emptyset$.

Then $\forall m: m \in [1, \ldots, M]$ *Level2 Order Function* $o^{p_0, m}(p,t)$ defined as:
   
$$ o^{p_0, m}(p,t) = \begin{cases}o^{\mathcal{L}}(p_0,t), & (p,t) \in \{p_0\} \times I_m \\ 0, & \text{otherwise} \end{cases}
  $$ is Order Function as defined by \@ref(def:order). 
```

```{proof Level2 order proof}
To prove the theorem we need to construct $\mathcal{T}$, $\mathcal{P}$ and $\mathcal{V}$ sets which are used in definition \@ref(def:order).

Suppose that $\{t_r^m\}_{r=1}^R \in I_m$ are points of dicontinuity of $o^{p_0,m}(p_0,t)$. 

If $R=1$ then $\mathcal{T} = \{(t^m_1, +\infty)\}$, $\mathcal{P} = \{\{p_0\}\}$, $\mathcal{V} = \{o^{p_0,m}(p_0,t^m_1)\}$.

Now suppose that $R >1$. We may assume without loss of generality that $t^m_i < t^m_j$ if $i < j$. Then $\forall r \in \{1, \ldots, R\}$ we'll construct $T_r \in \mathcal{T}$, $P_r \in \mathcal{P}$, $v_r \in \mathcal{V}$ as follows:

  a. $P_r = \{p_0\}$,
  a. $v_r = o^{p_0, m}(p_0, \frac{t_r+t_{r+1}}{2})$,
  a. $T_r$ is an interval with left endpoint $t_r$ and right-endpoint $t_{r+1}$; it is left-closed if $o^{p_0,m}(p_0, t_r) = v_r$ and left-open otherwise; it is right-closed if  $o^{p_0,m}(p_0, t_{r+1}) = v_r$ and right-open otherwise,
  
If $I_m$ is right-unbounded then $T_{R+1}$ with $t_R$ as its left-endpoint and $+\infty$ as its right-endpoint is added to $\mathcal{T}$, $P_{R+1} = \{p_0\}$, $v_{R+1} = o^{p_0, m}(p_0, t_R+1)$; $T_{R+1}$ is left-closed if $o^{p_0,m}(p_0, t_R) = v_{R+1}$ and left-open otherwise.

Finally, $\forall t^m_r : r > 1 \land t^m_r \not \in T_{r-1} \land t^m_r \not \in T_r$ a degenerate interval $\{t^m_r\}$ is added to $\mathcal{T}$, corresponding interval $\{p_0\}$ is added to $\mathcal{P}$ and corresponding value $o^{p_0, m}(p_0, t^m_r)$ is added to $\mathcal{V}$.

```

Table \@ref(tab:transmute-example-3) is a transmuted table \@ref(tab:transmute-example-2). Its visual representation shown on figure \@ref(fig:transmute-example-3-picture) is not distorted since order 5, which is Level2 Order Function, properly aggregates volumes of orders 3 and 4.

```{r transmute-example-3, echo=FALSE}
dt <- fread(file = "transmute_example_3.csv")[,c("maker","ordinal","timestamp","mks","state","price","volume","trade")]
setnafill(dt, type="locf", cols=3)
setnafill(dt, type="const", fill=0, cols=c(4, 5,7,8))
as_hux(dt[ , .(`\\(i\\)`=maker, `\\(k\\)`=ordinal, `\\(t^i_k\\)`=timestamp+mks/1000000.0+0.0000005,
               `\\(s^i_k\\)`=state, `\\(p^i_k\\)`=price, `\\(v^i_k\\)`=volume, `\\(m^i_k\\)`=as.integer(trade != 0) )]) %>%
  set_background_color(c(4,5,7,10), everywhere, "grey") %>%
  set_caption("Order 5 is now Level2 Order Function created from orders 3 and 4 in the previous table")

```


```{bash gnuplot-code-3, echo=FALSE}
gnuplot -p <<-EOF
  set xdata time
  set timefmt "%Y-%m-%dT%H:%M:%SZ"
  set xtics format "%M:%.1S" rotate by 45
  ...
  set cbrange [-2:2]
  set term png
  set output "transmute_example_3.png"
  set grid
  set pointsize 2
  val(name) = stringcolumn(name) ne "" ? column(name) : 0
  tval(t)=stringcolumn("timestamp") ne ""? strptime("%Y-%m-%dT%H:%M:%SZ",stringcolumn("timestamp")):t
...
```{r transmute-example-3-picture, echo=FALSE, fig.cap="A visual representation of Order Book Set \\@ref(tab:transmute-example-3) contains Level2 Order Function created from orders 3 and 4 from table \\@ref(tab:transmute-example-2)", fig.align="center"}
knitr::include_graphics("transmute_example_3.png", dpi=80)
```
...
`main` function of the application is rather short:
...
Figure \@ref(fig:Main) shows the specification of `oberon::App::main` operation in the form of UML2 Activity Diagrams [@cook_omg_2017] of activity `Main`. 


```{r Main, echo=FALSE, fig.cap="Activity `Main` - the specification of `oberon::App::main` operation.", fig.align="left"}

knitr::include_graphics("html_docs/fig644614.png", dpi=80)
```
CallOperationAction `Parse options` calls `Main::parseOption` operation that parse command-line options and returns an instance of a class derived from `Command` shown on figure \@ref(fig:Command) in light-green as determined by the value of "command" parameter passed. 

CallOperationAction `Create reconstructor and acquirer` then calls operation `Command::createAcquirer` using the received `Command`-derived instance as a target. The overriden `createAcquirer` operation returns an appropriate instance of class derived from `Acquirer` shown on figure \@ref(fig:Acquirer) in light-green.

The `Acquirer`-derived instance is then used as a `target` by CallOperationAction `AcquireAndProcess` that calls `Acquirer::acquireAndProcess` operation.

There are two kinds of `acquireAndProcess` operation. Their specifications are shown on figures \@ref(fig:receiver-acquireAndProcess) and \@ref(fig:reader-acquireAndProcess). The former is used when `Create reconstructor and acquirer` returns an instance derived from `Receiver` class and the latter is used when the instance is derived from `Reader` class as shown on figure \@ref(fig:Acquirer).

Upon successful completion of `acquireAndProcess` operation, ValueSpecificationAction `Success` returns EXIT_SUCCESS constant.

Otherwise, if some exception was raised, ValueSpecificationAction `Failure` returns EXIT_FAILURE constant.

`Main` activity is then completes.


### Parse options

Action node "Parse options" on figure \@ref(fig:Main) returns a `unique_ptr` to an instance of concrete class (shown on figure \@ref(fig:Command) in light green) derived from the abstract class `Command`. As it can bee seen the concrete class is derived either from `Capture` or from `Transform`.

```{r Command, echo=FALSE, fig.cap="Command class hierarchy", fig.align="left"}

knitr::include_graphics("html_docs/fig691206.png", dpi=80)
```

### Create reconstructor and acquirer


#### Create acquirer

Action node "Create reconstructor and acquirer" on figure \@ref(fig:Main) calls `createAcquirer()` method on the instance of `Command` class received on `target` input pin. The method calls the protected `createReconstructor()` method internally and returns  a `shared_ptr` to an instance of concrete class derived from the abstract class `Acquirer` shown on figure \@ref(fig:Acquirer) in light green.


```{r Acquirer, echo=FALSE, fig.cap="Acquirer class hierarchy", fig.align="left"}

knitr::include_graphics("html_docs/fig704262.png", dpi=80)
```
 The specification of  `Receiver::acquireAndProcess` operation is provided in section \@ref(receiver-acquire-and-process).  The specification of `Reader::acquireAndProcess` operation is provided in section \@ref(reader-acquire-and-process).

#### Create reconstructor {#create-reconstructor}

This section provides specifications of `createReconstructor` operation of concrete classes shown on figure \@ref(fig:Command) in light-green.

##### coinbase::Capture


```{r create-reconstructor-coinbase, echo=FALSE, fig.cap="Specification of `coinbase::Capture::createReconstructor` operation", fig.align="left"}

knitr::include_graphics("html_docs/fig710918.png", dpi=80)
```

##### bitstamp::Capture

```{r create-reconstructor-bitstamp, echo=FALSE, fig.cap="Specification of `bitstamp::Capture::createReconstructor` operation", fig.align="left"}

knitr::include_graphics("html_docs/fig717574.png", dpi=80)
```
##### Slice
...
```{r create-reconstructor-transmuter, echo=FALSE, fig.cap="Specification of `Transmute::createReconstructor` operation", fig.align="left"}

knitr::include_graphics("html_docs/fig598790.png", dpi=80)
```
### Acquire and process

This section provides specifications of `acqureAndProcess` operation of concrete classes shown on figure \@ref(fig:Acquirer) in light-green. Note that both `Receiver`-derived classes share `acquireAndProcess` operation defined in `Receiver` class.

#### Receiver{#receiver-acquire-and-process}


`Coinbase::Receiver` and `Bitstamp::Receiver` classes inherit `acquireAndProcess()` method from `Receiver` class. Its activity diagram is shown on figure \@ref(fig:receiver-acquireAndProcess). 

```{r receiver-acquireAndProcess, echo=FALSE, fig.cap="Specificationf of `Receiver::acquireAndProcess()` operation", fig.align="left"}
knitr::include_graphics("html_docs/fig670854.png", dpi=80)
```


#### Reader{#reader-acquire-and-process}


`Reader` class has another `acquireAndProcess()` method. Its activity model is shown on figure \@ref(fig:reader-acquireAndProcess).

```{r reader-acquireAndProcess, echo=FALSE, fig.cap="Specificationf of `Reader::acquireAndProcess()` operation", fig.align="left"}
knitr::include_graphics("html_docs/fig605318.png", dpi=80)
```

### Receive messages

Action `Receive messages` on figure \@ref(fig:receiver-acquireAndProcess) is UML2 `StartObjectBehaviorAction` that starts execution of `Receiver` StateMachine shown on figure \@ref(fig:receiver). The call is synchronous, so the execution of the `Receive messages` does not complete until the execution of `Receiver` StateMachine completes.

(ref:receiver-caption) `Processing snapshot` and `Processing messages` states pass received portions of data asynchronously to an instance of `Reconstructor` (by calling operation `Reconstructor::process()` shown on figure \@ref(fig:process-message)) thereby the streaming of data between `Receive messages` and `Process message` on figure \@ref(fig:receiver-acquireAndProcess) is implemented.


```{r receiver, echo=FALSE, fig.cap="(ref:receiver-caption)", fig.align="left"}
knitr::include_graphics("html_docs/fig399494.png", dpi=80)
```
...
knitr::include_graphics("html_docs/fig406150.png", dpi=80)
```

(ref:snapshot-caption) Snapshot StateMachine diagram.

```{r snapshot, echo=FALSE, fig.cap="(ref:snapshot-caption)", fig.align="left"}
knitr::include_graphics("html_docs/fig406534.png", dpi=80)
```
### Process message

Activity `Process message` shown on figure \@ref(fig:process-message) is a specification of `Reconstructor::process` operation.

```{r process-message, echo=FALSE, fig.cap="Specification of Reconstructor::process() operation that is called by Process message node", fig.align="left"}
knitr::include_graphics("html_docs/fig704390.png", dpi=80)
```

It takes an instance of  [Boost.PropertyTree](https://www.boost.org/doc/libs/1_73_0/doc/html/property_tree.html) container with an exchange-specific data as `message` parameter and passes it to `Extract message` action. The action creates then an instance of one of the exchange-independent class, derived from `Message`. Possible classes are shown in green on figure \@ref(fig:message-hierarchy-process). The instance is returned in `Messages` container. The cointainer is a list of unique pointers `Message_UP` to `Message`-derived objects. 

```{r message-hierarchy-process, echo=FALSE, fig.cap="`Extract` returns an instance of class `Messages`", fig.align="center"}
knitr::include_graphics("html_docs/fig552070.png", dpi=80)
```

The further processing of messages by `MessageHandler`s is implemented using [Chain of Responsibility](https://en.wikipedia.org/wiki/Chain-of-responsibility_pattern) pattern.  `Messages` container is passed to ExpansionRegion of type "iterative" as `input` parameter as shown on figure \@ref(fig:process-message). The region calls `MessageHandler::handle` operation on the instance of `MessageHandler` currently available within the region for each `Message_UP` in `input`. The outputs are combined and passed to the next `MessageHandler` returned by `next` transformation, if any (this instance becomes available for the next iteration). The cycle repeats until either `input` or next `MessageHandler` is empty.

When created, an instance of `MessageHandler` takes, as a constructor parameter, a shared pointer to the next `MessageHandler` which will receive messages as soon as `MessageHandler` being constructed has handled them. Thus the chain is static, i.e. it does not change depending on messages received. 

The kind of processing performed by the chain of `MessageHandler`s depends on command being performed.

For command "capture" the chain performes so called *data cleansing*. According to [@rahm_data_2000]:
...
The exact list of `MessageHandler`s needed for that depends on exchange. The lists are described in section \@ref(create-reconstructor).


### Handle message

UML2 Sequence Diagram of `MessageHandler::handle()` is shown on figure \@ref(fig:sd-Messagehandler-handle). 

```{r sd-Messagehandler-handle, echo=FALSE, fig.cap="UML2 Sequence diagram showing a \"happy path\" trace of `MessageHandler::handle()` method.", fig.align="left"}
knitr::include_graphics("html_docs/fig551814.png", dpi=80)
```


Handling of messages by `MessageHandler` is implemented using [Visitor](https://en.wikipedia.org/wiki/Visitor_pattern) pattern as shown by UML2 Sequence diagram on figure \@ref(fig:sd-message-accept).

```{r sd-message-accept, echo=FALSE, fig.cap="UML2 Sequence diagram showing various alternatives of traces for Message::accept() method. By default all `MessageHandler`s callback (i.e. `era`, `message` etc) just pass the message to `output_` and it is expected that derived classes override the callbacks in order to do something useful. The message can be saved by `MessageHandler` instance in order to be processed later together with messages that follows.", fig.align="left"}
knitr::include_graphics("html_docs/fig545286.png", dpi=80)
```


The result of `handle` is a set of zero or more messages that are stored in `output_` member variable that is an instance of `Messages` class shown on figure \@ref(fig:message-hierarchy-process). `MessageHandler` will not produce output for a message when subsequent messages are required for handling. In this case the message will be kept within this particular `MessageHandler` instance between runs of `Reconstructor::process()` until enough subsequent messages have been received. Then all kept and handled messages are returned together, saved into `output_` variable.

### Output format

#### File

MessageHandler `File` saves the received events into files. Upon receiving of a message of class `Era` (see figure \@ref(fig:message-hierarchy-process)) `File` opens a new file called *Era file* and saves the subsequent `OrderEvent` messages there until obtaining `Reset` message when the handler closes Era file. `File` drops `Elapsed` messages silently.

Era file is [CSV-file](https://en.wikipedia.org/wiki/Comma-separated_values) with one row per `OrderEvent` and its name is constructed from  `exchange`, `product` and `timestamp` values carried by `Era` message. Row fields are described in the table \@ref(tab:order-record) below.
