...
```{definition LOB function, name="LOB functions", label="lob-functions"}
Suppose Limit Order Book $\mathcal{L}$ as defined by \@ref(def:lob) is given, $\mathcal{B}$ is an associated *bids* set as defined by \@ref(def:bids) and $\mathcal{A}$ is an associated *asks* set as defined by \@ref(def:asks). The function
$$
  o^{\mathcal{B}}(p,t) = \sum_{o^i(p,t) \in \mathcal{B}} o^i(p,t) \\
  o^{\mathcal{A}}(p,t) = \sum_{o^i(p,t) \in \mathcal{A}} o^i(p,t) \\
  o^{\mathcal{L}}(p,t) = o^{\mathcal{A}}(p,t) + o^{\mathcal{B}}(p,t)
$$
is called *LOB bid function* , *LOB ask function* and *LOB function* respectively. 
```

Note that $$\text{supp}(o^{\mathcal{A}}(p,t)) \cap \text{supp}(o^{\mathcal{B}}(p,t)) = \emptyset$$.
...
```{theorem Level2 order, name="Level2 Order Function", label=l2of}
    Suppose Limit Order Book $\mathcal{L}$ as defined by \@ref(def:lob) is given,  $o^{\mathcal{L}}(p,t)$ is LOB function as defined by \@ref(def:lob-functions) and for some constant $p_0 \in \mathbb{R}_{>0}$:  $$\emptyset \neq \text{supp}(o^{\mathcal{L}}(p_0,t)) = \bigcup_{m=1}^M I_m$$ where each $I_m$ is an interval.
