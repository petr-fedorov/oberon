...
DataFrame RCoinbase::run(const Rcpp::DataFrame & data) {
  ...
  return Rcpp::DataFrame::create(
      Rcpp::Named("order_id") = order_id_, Rcpp::Named("event_no") = event_no_,
      Rcpp::Named("timestamp") = timestamp_,
      Rcpp::Named("event_type") = event_type_, Rcpp::Named("price") = price_,
      Rcpp::Named("remaining_size") = remaining_size_,
      Rcpp::Named("change_size") = change_size_,
...
void RCoinbase::transmit( std::unique_ptr<oberon::core::Event> && event) {
  if(event) {
    ...
    remaining_size_.push_back(event->getRemainingSize());
    change_size_.push_back(event->getChangeSize());
    trade_id_.push_back(event->getTradeId());
